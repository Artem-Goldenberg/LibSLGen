...
fun TransitionGraph.randomWalkFrom(node: State): ConsList<TransitionGraph.Edge> {
    ...
    val randomPoint = Math.random() * totalPriority
    var point = 0
    for (edge in edges) {
        point += edge.priority.value
        if (point >= randomPoint) {
            val path = randomWalkFrom(edge.to)
...
fun TransitionGraph.augmentWithRandomWalks(paths: List<Path>): List<Path> = paths.map { path ->
    when (val node = path.getLast()) {
        null -> path
        else -> {
            val walk = this.randomWalkFrom(node.to)
